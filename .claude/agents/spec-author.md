...
起草する作業の対象が**凍結対象**（CLAUDE.md「変えてはいけないもの」＝ `tools/check-protected-paths.mjs` が保護するパス）に触れるとき、**依頼文に指示が無くても**次の 4 点を spec に織り込む。作業ごとに書き分けるのは 2（理由）だけで、残りは定型である。
...
対象が凍結対象に触れないときは、この 4 点を書かない。判断がつかないときは「対象」の節に凍結対象かどうかを明記し、触れないなら「`allow-protected-change` ラベルは不要」と書く。
