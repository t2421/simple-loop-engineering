...
# Figma を抽出して progress/ に置く

Figma のライブファイルは完了条件にしない。**抽出して `progress/` に置いた JSON・PNG が正である。**
...
進捗ファイルと同じディレクトリ・同じベース名にする。

| 対象 | パス |
|---|---|
| 進捗 | `progress/<作業>.md` |
| トークン | `progress/<作業>.figma.json` |
| 見た目 | `progress/<作業>.png` |

アーカイブするときは進捗といっしょに `progress/archive/` へ移す。
...
抽出した JSON は、描画して計算スタイルを読むテストの期待値になる。テストは `npm run ci` が回す。progress には書かない。

PNG はピクセル差分の基準になる。差分画像を出す場合は gitignore する（`progress/<作業>.diff.png`）。
