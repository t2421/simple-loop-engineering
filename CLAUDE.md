...
守りたいものが増えたら、次の順で行う。**文章だけ足してもガードは検知しない。** 一覧と判定ロジックの両方を直す。

1. **何を防ぐのかを一文で決める。** 「検証を弱める変更」を止めるのが目的である。単に触られたくないだけのファイルは対象にしない
2. **この節の一覧に行を足す**
3. **`tools/check-protected-paths.mjs` の判定に足す。** 形は 3 つある
   - ディレクトリごと守る → `APPEND_ONLY_DIRS` に足す。内容が同一のままの移動（アーカイブ）を許すかを `archiveMove` で決める
   - 1 ファイルだけ守る → `CHECKER` と同じ形の定数にする
   - ファイルの中の特定の値だけ守る → `package.json` の `scripts` と同じく、base と head の値を比べる専用の判定を書く
4. **`tests/protected-paths.test.mjs` にケースを足す。違反側だけでなく許可側も書く。** 新規追加・内容同一の移動・保護ディレクトリの外へのリネーム・削除。許可側を書かないと、正当な操作まで止めるガードになる
5. **`specs/guard-protected-paths.md` の「仕様」の列挙にも足す。** これは spec の変更なので、[コミットとマージ](#コミットとマージ) のとおり変更内容と理由を試行ログに記録し、人間の承認を経る
6. **その PR に `allow-protected-change` ラベルを付ける。** spec と判定ロジックを触る以上、自分のガードに引っかかる。これは正しい動作である

**新しい保護が効き始めるのは、その PR がマージされた後の PR からである。** ガードは base リビジョンの `tools/check-protected-paths.mjs` を実行するので、保護を足す PR 自身は古い判定で評価される。導入 PR で新しい保護が効かないのは想定どおりで、異常ではない。

保護を外すときも同じ手順を踏む。外す理由を spec に書き、人間の承認を経る。
