...
- **派生 docs PR**（作業の途中で見つかった規約衝突などから、計画していなかった docs 改訂を独立 PR にするとき）を新規に開く直前に、オープンな PR を確認する。確認する材料はタイトル・本文・ブランチ名・変更対象である。直そうとしている対象（同じファイルの同じ節、または同じ発見から導いた同じ種類の docs 改訂）が一致すれば、文言が違っても**同じ意図**である。同じ意図のオープン PR が既にあれば新規に作らない。既存 PR にコメントして合流する（必要ならそのブランチに差分を足す）
- 重複した派生 docs PR は、人間の指定が無ければ先にオープンされた PR を残す。後発の側が自分の PR を close し、残す PR へのリンクと重複である旨をコメントする。後発にだけある有意な差分は close 前に残す側へ取り込む。人間が残す側を明示したらそれに従う
...
### 同一ブランチで docs 作業を並行するとき

backlog リファインメントのように、複数エージェントが同一チェックアウト・同一ブランチで docs を並行して触ることがある。worktree を分けないぶん、共有インデックスへの並行 `git add` が誤ステージ・誤コミットを生む。次を守る。

- **親（オーケストレータ）が各エージェントに触ってよいパス（担当ディレクトリ・ID のリスト）を明示的に割り当てる。** サブエージェントは割当外のパスを変更しない
- **`git add`・`git commit`・`git mv` は親だけが行う。** サブエージェントはファイルの作成・編集までとする
- **サブエージェントは `git status` 全体を根拠に判断しない。** 他のエージェントの途中状態（未追跡ディレクトリ・部分ステージ）が混ざる

実装作業（コードを触る作業）はこの例外に含まない。上の「1 worktree = 1 作業 = 1 ブランチ」に従う。

### 派生 docs PR の重複

別々の作業をしていたセッションが、同じ発見から同じ派生 docs PR を二重に作ることがある。担当ディレクトリの割当では防げない。手段は手順の規約だけとする。実装 PR（`task/` の作業ブランチ）の重複はこの手順の対象外である。

- **作成前の確認:** 派生 docs PR を新規に開く直前に、オープンな PR を確認する。確認する材料はタイトル・本文・ブランチ名・変更対象である
- **同じ意図:** 直そうとしている対象（同じファイルの同じ節、または同じ発見から導いた同じ種類の docs 改訂）が一致すること。文言が違っても対象と種類が同じなら同じ意図である
- **合流:** 同じ意図の派生 docs PR が既にオープンなら、新規に作らない。既存 PR にコメントして合流する（必要ならそのブランチに差分を足す）
- **畳み方（人間の指定が無いとき）:** 重複した派生 docs PR は、残すのは先にオープンされた PR。後から作った側が自分の PR を close し、残す PR へのリンクと重複である旨をコメントする。後発にだけある有意な差分は close 前に残す側へ取り込む
- **人間の指定が優先:** 人間が残す側を明示したらそれに従う
...
- backlog spec を**新規に起こすとき・実測して更新するとき**は、「背景」冒頭の backlog 定型行（「この項目は backlog。着手しない。…」）の**直後**に、実測の 1 行を置く（既存の候補への遡及適用は求めない）。書式は `実測: YYYY-MM-DD — <確認した事実の 1 行>`。据え置き中なら、何の実測を待っているかをその 1 行に含める
- **再実測したらこの行を書き換える。行を積み増さない。** 履歴は git が持つ。散文に実測を積み重ねると、次のリファインメントで「いつ時点の観測か」を spec 全体から拾い直すことになる
...
- 実測の CI 結果には、取得時点のコミット SHA を添える。その SHA はレビュー対象の差分の HEAD と一致していなければならない。未コミットの変更がある状態で取得した結果は、その旨を添える
- 実測結果の SHA とレビュー対象 HEAD の一致が確認できないとき（SHA 欠落、不一致、未コミットの旨の欠落、曖昧）は承認しない。取り直しを求める
