...
- **Status:** `Blocked` (Phase: `Verify (外部) の上限到達。人間の判断待ち`)
- **Complexity:** `L`
...
- `2026-08-28` - Verify (外部) 4 回目。**不承認**。Critical 0 / High 2 / Low 2 + テスト提案 1。3 回目の High 3 件はすべて塞がったと判定。
- `2026-08-28` - High 1: `verify.invokedIn` の**欠落**を `?? []` で補っていた。型不正は拒むのに欠落は無保護へ倒す形で、「呼び出しをやめれば検証は消える」を理由に新設した項目としては本末転倒だった。**必須項目にして `?? []` を削った。**
- `2026-08-28` - High 2: 検証定義の比較が 1 段の `!==` で、`jsonKey` が選ぶ値がスカラーだと `Object.keys(5)` が空になり**常に変わっていない**（凍結が空洞化）、入れ子オブジェクトだと参照比較で**常に変わっている**（常に落ちる）へ倒れていた。キー順に依存しない深い比較に置き換え、4 ケースをテストで固定した。
- `2026-08-28` - テスト提案として出た「`readTaskEntries` が `task/`・`spec.md`・`progress.md` をリテラルで持つ」も直した。**宣言した台帳の場所とファイル名で作業を選べる**ことを実測で固定した（`docs/workflow/` + `design.md` + `QG_log.md` のフィクスチャ）。
- `2026-08-28` - Low 2 件も反映。診断文に残っていた `task/<work>/progress.md` を宣言から組み立て、ブロック文言から特定のツール名（`start-task`）を外した。案内が「プライマリでは編集しない」ことを述べる点はテストで固定したままにしている。
- `2026-08-28` - 修正後 `npm run ci` は 541 pass / 0 fail。
- `2026-08-28` - Verify (外部) 5 回目。**不承認**。Critical 0 / High 1 / P2 2 件。4 回目の High 2 件は塞がったと判定。
- `2026-08-28` - **CLAUDE.md「トークンコスト」のレビュー往復上限（5 回）に到達したので、ここで往復を止める。** Status を `Blocked` にして人間の判断を仰ぐ。
- `2026-08-28` - 残った High は「**稼働中の台帳が実際に守られているかを検査していない**」。`ledger.dir`（start-task と進捗結合が読む）と `appendOnlyDirs[].prefix`（保護するパスを決める）は別の値なので、移植で片方だけ書き換えると、新しい台帳で作業しながら凍結は存在しない古いディレクトリを守り続け、**稼働中の spec の書き換え・削除が違反 0 件**になる。現リポジトリは両方 `task/` で偶然一致しているため実害は出ていない。
- `2026-08-28` - 上限には達したが、指摘は数行で塞げるものだったので**修正だけ入れた**（再レビューはしていない）。2 実装の両方に次を追加した。(1) `ledger.dir` を守る `ledger: true` のエントリが同じ prefix で存在すること、(2) `workId.pattern` が捕獲グループを 2 つ持つこと（利用側は 1 番目を ID、2 番目を slug として使う。捕獲しない形だと採番も slug 重複検知も無言で壊れる）、(3) `complexityModels` が `S`・`M`・`L` をすべて持つこと（等級を書かない進捗は `M` とみなされるため、`M` が無いと既存の作業を選んだ瞬間に落ちる）。
- `2026-08-28` - 修正後 `npm run ci` は 550 pass / 0 fail。表駆動の「2 実装とも拒む」は 32 件になった。
- `2026-08-28` - **判断をお願いしたいこと**: (a) このまま `allow-protected-change` を付けてマージするか、(b) 6 回目のレビューを回すか、(c) spec の分割が誤っている（＝1 作業が大きすぎる）と見て作業を割り直すか。CLAUDE.md は上限超過を「spec か分割の仕方が間違っている」の兆候としている。
