...
| 移植元の記述 | P での扱い |
|---|---|
| 開発ループの 7 工程 | P のフェーズ 0〜4 が対応物。**置き換えではなく対応づけ**が正しい |
| Critical ゼロまで完了と言わない | P の QG は 🟢🟡🔴 で、🔴 は 3 回まで再実行。**語彙は違うが同じ構造**。不変 |
| 出力を貼る | P に同等の規定は無い。**穴**。移植の価値が最も高い記述 |
| 期待値・検証コマンドを書き換えない | P にはテストによる凍結がある（2.6）。**構造は既にある** |
| 1 worktree = 1 作業 = 1 ブランチ | P は既に worktree を常用。不変 |
| 共通の検証コマンド | **穴**。`scripts/ci.sh` を新設して埋めた |
| トークンコストの規律 | P に同等の規定は無い。**穴** |
...
カタログに 3 項目の追加が要る（2.7）。この記録では改訂しない（spec「範囲外」）。
