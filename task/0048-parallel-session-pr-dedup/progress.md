...
- `00:49` - `npm run lint:docs`: exit 0
...
- `03:11` - `npm run ci`: exit 0（HEAD `ba577f0`）
...
- `05:04` - 共通検証 dump をこの progress から除いた（#104 の `checkProgressNoSharedVerification` 向け）。docs lint 成功文と `npm run ci` の全件集計は残さない。`grep -n "派生"` と CLAUDE.md 追記の記録は残した。
