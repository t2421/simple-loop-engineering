...
- `05:12` - Copilot #102 再指摘。`isAllowedCommand` が grep/wc の絶対パスと `..` セグメントを拒否。`splitPipelineStages` は空段（`echo 1 | | wc` / 末尾 `|`）を失敗にする。実行せず拒否するテストを追加。`node --test tests/check-examples.test.mjs` は `# tests 21` / `# pass 21` / `# fail 0`。`npm run ci: exit 0`。
